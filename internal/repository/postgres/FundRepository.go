...
func (r *FundRepository) Create(ctx context.Context, fund *domain.Fund) (*domain.Fund, error) {
	...
	_, err := r.DB.Exec(ctx, `INSERT INTO funds
    (name, author_id, invite_code) 
	VALUES ($1, $2, $3) 
	ON CONFLICT DO NOTHING
	RETURNING id, created_at`, fund.Name, fund.AuthorID, fund.InviteCode, fund.ID, fund.CreatedAt)
...
func (r *FundRepository) GetByUserID(ctx context.Context, userID int64) ([]domain.Fund, error) {
	var funds []domain.Fund
	query, err := r.DB.Query(ctx, `SELECT (id, name, author_id, invite_code, created_at) FROM funds WHERE author_id = $1`, userID)
	if err != nil {
		return nil, err
	}
	for query.Next() {
